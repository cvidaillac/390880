...
= Pop3PLyreGetEnergiesRestAPI
REST API to retrieve the list of enegies from Lyre

The project **pop3PLyreGetEnergiesRestAPI** is a Bonita REST API extension for **Bonita 7.12.4** version  **community** written in `java`
...
[source,bash]
----
git clone https://your.github.com/pop3PLyreGetEnergiesRestAPI.git
cd pop3PLyreGetEnergiesRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `pop3PLyreGetEnergiesRestAPI-1.0.0.zip`
...
Take the built artifact `pop3PLyreGetEnergiesRestAPI-1.0.0.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
