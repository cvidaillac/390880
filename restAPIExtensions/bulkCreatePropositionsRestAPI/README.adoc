...
= BulkCreatePropositions
REST API to manage resourceName

The project **bulkCreatePropositionsRestAPI** is a Bonita REST API extension for **Bonita 7.12.4** version  **community** written in `java`
...
[source,bash]
----
git clone https://your.github.com/bulkCreatePropositionsRestAPI.git
cd bulkCreatePropositionsRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `bulkCreatePropositionsRestAPI-3.1.0-${target.environment}.zip`
...
Take the built artifact `bulkCreatePropositionsRestAPI-3.1.0-${target.environment}.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
