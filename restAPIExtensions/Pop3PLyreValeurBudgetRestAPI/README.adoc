...
= pop3PLyreValeurBudget
REST API to retrieve suggested values for Valeur Budget from Lyre Web service

The project **Pop3PLyreValeurBudgetRestAPI** is a Bonita REST API extension for **Bonita 7.12.4** version  **community** written in `java`
...
[source,bash]
----
git clone https://your.github.com/Pop3PLyreValeurBudgetRestAPI.git
cd Pop3PLyreValeurBudgetRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `Pop3PLyreValeurBudgetRestAPI-1.0.0.zip`
...
Take the built artifact `Pop3PLyreValeurBudgetRestAPI-1.0.0.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
