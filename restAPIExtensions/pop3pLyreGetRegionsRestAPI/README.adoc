...
= PopEPLyreGetRegions
REST API to get regions from Lyre

The project **pop3pLyreGetRegionsRestAPI** is a Bonita REST API extension for **Bonita 7.12.4** version  **community** written in `java`
...
[source,bash]
----
git clone https://your.github.com/pop3pLyreGetRegionsRestAPI.git
cd pop3pLyreGetRegionsRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `pop3pLyreGetRegionsRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `pop3pLyreGetRegionsRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
