...
= pop3PLyreBudgetPlan
REST API to call Lyre Web servicesq WSBP_SYNTH_PLAN_PREV and WSBP_PLAN_PREV

The project **pop3PLyreBudgetPlan** is a Bonita REST API extension for **Bonita 7.12.9** version  **community** written in `java`
...
[source,bash]
----
git clone https://your.github.com/pop3PLyreBudgetPlan.git
cd pop3PLyreBudgetPlan/
mvn package
----

The build should produce a zip archive under the `target/` folder named `pop3PLyreBudgetPlan-1.0.0-DEV.zip`
...
Take the built artifact `pop3PLyreBudgetPlan-1.0.0-DEV.zip` and upload it to your Bonita platform (see https://documentation.bonitasoft.com/bonita/${shortBonitaVersion}/rest-api-extensions#toc7)
