...
class IndexTest extends Specification {
	...
	/*
	 * def should_return_a_json_representation_as_result() { given:
	 * "a RestAPIController" def index = new Index()
	 * 
	 * when: "Invoking the REST API" def apiResponse = index.doHandle(httpRequest,
	 * new RestApiResponseBuilder(), context)
	 * 
	 * then: "A JSON representation is returned in response body" def jsonResponse =
	 * new JsonSlurper().parseText(apiResponse.response) // Validate returned
	 * response apiResponse.httpStatus == 200 jsonResponse.myParameterKey ==
	 * "testValue" jsonResponse.currentDate == LocalDate.now().toString() }
	 */
