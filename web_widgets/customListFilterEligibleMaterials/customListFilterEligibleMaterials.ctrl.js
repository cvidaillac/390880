function listFilters($scope, $log) {
    ...
    this.getMultiSelectAttribute = function(options_values, attribute_name) {
        var selected_values='';
        try {
            if (options_values) {
                for (var one_selected_index of this.multiSelectIndexes) {
                    if (one_selected_index < options_values.length) {
                        ...
                        if (selected_values) {
                            selected_values += this.summaryMultipleValuesSeparator;
                        }
                        selected_values += one_selected[attribute_name];
                    } else {
            ...
            for (var i=0; i<nb_filters ; i++) {
                ...
                if (one_filter.type === 'selectMulti') {
                    if (this.multiSelectIndexes) {
                        one_value = this.getMultiSelectAttribute(one_filter.values, 'label');
                    }
                } else {
        ...
        try {
            if (this.multiSelectIndexes) {
                ...
                for (var i=0; i<nb_filters ; i++) {
                    var one_filter = $scope.properties.filtersLabels[i];
                    if (one_filter.type === 'selectMulti') {
                        $scope.searchBuffer[one_filter.data] = this.getMultiSelectAttribute(one_filter.values, 'value');
    ...
    this.setCheckboxSummary = function() {
        try {
            ...
            ctrl.checkboxSummaryText = "Cause" + plurial + " d'éligibilité : " + summary;
        } catch(e) {
    ...
    this.generateCustomUrl = function generateCustomUrl() {
        ...
        angular.forEach($scope.searchBuffer, function(value, key) {
            ...
            if(value !== ""){
                ...
                urlToGenerate += "&"+ operatorSearchKey +"=" + key + "=" + encodeURIComponent(value);
