function PbDataTableCtrl($scope, $http, $log, $filter) {
  ...
  function resetPaginationAndUpdateResults() {
    ...
    $scope.toggleObjSelection = function(row, $event){
        $event.stopPropagation();
        if (row.statusRow){
            idsSelected.push(row.id)
        }else {
            var index = idsSelected.indexOf(row.id);
    ...
    $scope.validateAll = function(){
        //Boucle sur tous les ids pour soumettre les taches
        for (var i=0; i <idsSelected.length; i++){
            var url = " ../API/bpm/userTask/"+idsSelected[i]+"/execution";
            var sentObject = {};
  ...
  //Fonctions Custom pour traiter la tache
  $scope.approve = function(ligne, $event){
      //Empeche le selectedRow d'etre propagé :
     $event.stopPropagation();
      
      var url = " ../API/bpm/userTask/"+ligne.id+"/execution";
      var sentObject = {};
