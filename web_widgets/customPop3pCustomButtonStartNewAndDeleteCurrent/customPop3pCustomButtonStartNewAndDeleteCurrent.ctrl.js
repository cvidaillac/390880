function PbButtonCtrl($scope, $http, $location, $log, $window) {
  ...
  this.action = function action() {
    ...
    if ($scope.properties.action === 'Remove from collection') {
      ...
      if (id) {
        ...
        console.log("TASK id : " +  $scope.properties.taskId);
        console.log($scope.properties.formOutputRefus);        
        // Start a new one
        doRequest($scope.properties.action, $scope.properties.url, $scope.properties.dataToCreateNew, null, false);


        // Cancel the current task
        doRequest('POST', '../API/bpm/userTask/' + $scope.properties.taskId + '/execution', $scope.properties.dataToRefuse, null, false);
   ...
   * @return {void}
   */
  function doRequest(method, url, dataToSend, params, reloadLocation) {
    vm.busy = true;
    var req = {
      method: method,
      url: url,
