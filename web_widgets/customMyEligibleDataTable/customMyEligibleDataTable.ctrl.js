...
// Custom variables
	...
	//Fonctions Custom pour ouvrir un bon de commande sur calypso
	vm.createNewProposition = function (row, $event, $window) {
		...
		url += "PO_ID="+row.CDA;
		url += "&VENDOR_ID"+$scope.properties.codeFou
	//	url += "&SG_SRVC_ID=DSI&Action=U&cmd=login&languageCd=FRA&userid=VP1&pwd=VP1ZZZZ";
	
		

        //Redirect to Calypso PO Page
	...
	//Fonctions Custom pour ouvrir un bon de commande sur calypso
	vm.openContrat = function (row, $event, $window) {
		//Empeche le selectedRow d'etre propagé :
		$event.stopPropagation();

        //Prepare the URL
		var url = row.URL_CONTRAT;
        //Redirect to Calypso PO Page
	//	window.location.href = url;
		window.open(url, '_blank');
	}
...
//-----Custom developments---------------------------------------------------------------------//
	...
	this.checkAll = function () {
		...
		angular.forEach($scope.ctrl.results, function (row) {
			...
			if ($scope.selectAll) {
				...
				vm.addId(row.CDA)
                $scope.properties.listBCChecked=$scope.listIds;
	...
	vm.setListIds = function($event, $window) {
		$event.stopPropagation();
        $scope.properties.listBCChecked=$scope.ctrl.results;
	}

    this.displayConfirmation = function(title, content, type, calbackOnSuccess) {
    ...
    this.popUpApprove = function(row, $event) {
		$event.stopPropagation();
		var MyCtrl = this;
        this.displayConfirmation(
            'Confirmation de validation', 
            'Vous avez sélectionné le bon de commande N° ' + row.CDA + '. Confirmer la validation?',
            "blue", function() {MyCtrl.approve(row, $event)})
    }
    ...
    this.popUpvalidateAllSelected = function() {
        this.displayConfirmation(
            ...
            'Vous avez sélectionné ' + $scope.listIds.length + ' Bons de commandes(s). Confirmer les validations?',
