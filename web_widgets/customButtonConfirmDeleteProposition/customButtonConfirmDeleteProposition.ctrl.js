function PbButtonCtrl($scope, $http, $location, $log, $window) {
  ...
  this.action = function action() {
        ...
        this.displayConfirmation(
            ...
            $scope.properties.currentPropositionId ? 'Vous avez sélectionné la proposition ' + $scope.properties.currentPropositionId + '. Confirmer la supression?' : "Voulez-vous supprimer cette proposition ?",
    ...
    this.remove = function () {
        if ($scope.properties.collectionToModify) {
          ...
          if ($scope.properties.collectionPosition === 'First') {
            ...
            console.log("isExisting = " + $scope.properties.currentProposition.isExistring);
          // Only remove element for valid index
          if (index !== -1 ) {
              if ($scope.properties.currentProposition.isExistring === false) {
                  $scope.properties.collectionToModify.splice(index, 1);
                  $scope.$apply();
              } else {
                  $scope.properties.currentProposition.isRemove = true;
                  $scope.$apply();
              }
          }
